...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-2xl font-bold text-gray-800">Nuevo Aporte de Socio</h2>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <form method="POST" action="{{ route('caja.aportes.store') }}">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                ...
                <select name="socio_id"
                    ...
                    @foreach ($socios as $s)
                        <option value="{{ $s->id }}" {{ old('socio_id') == $s->id ? 'selected' : '' }}>{{ $s->nombres }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="periodo_mes"
                    ...
                    @foreach (range(1, 12) as $m)
                        <option value="{{ $m }}" {{ old('periodo_mes') == $m ? 'selected' : '' }}>{{ $mesNombres[$m] }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="periodo_anio" value="{{ old('periodo_anio', date('Y')) }}"
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" step="0.01" min="0" name="valor_cuota" value="{{ old('valor_cuota') }}"
            ...
            <div>
                ...
                <select name="estado_pago"
                    ...
                    <option value="exento" {{ old('estado_pago') == 'exento' ? 'selected' : '' }}>Exento</option>
                </select>
